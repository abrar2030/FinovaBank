...
class RiskAssessmentEngine:
    ...
    def calculate_credit_score(self, customer_data: Dict) -> Dict[str, Any]:
        ...
        payment_score = max(0, min(100, (on_time_payments / total_payments) * 100 - late_payments * 5))
    ...
    def assess_loan_risk(self, loan_application: Dict) -> Dict[str, Any]:
        ...
        dti_ratio = (monthly_debt + loan_payment) / monthly_income if monthly_income > 0 else 1
...
def calculate_credit_score():
    ...
    try:
        ...
        logger.info(f"Credit score calculated for customer {data.get('customer_id')}: {result['credit_score']}")
...
def assess_loan_risk():
    ...
    try:
        ...
        logger.info(f"Loan risk assessed for application {data.get('application_id')}: {result['risk_level']}")
...
def calculate_default_probability():
    ...
    try:
        ...
        logger.info(f"Default probability calculated for customer {data.get('customer_id')}: {result['default_percentage']}%")
...
def assess_portfolio_risk():
    ...
    try:
        ...
        high_risk_count = len([l for l in portfolio_results if l['risk_level'] in ['HIGH', 'VERY_HIGH']])
        avg_default_prob = np.mean([l['default_probability'] for l in portfolio_results])
        ...
        response = {
            ...
            "risk_distribution": {
                ...
                "medium": len([l for l in portfolio_results if l['risk_level'] == 'MEDIUM']),
                "high": len([l for l in portfolio_results if l['risk_level'] == 'HIGH']),
                "very_high": len([l for l in portfolio_results if l['risk_level'] == 'VERY_HIGH'])
            },
            "loans": portfolio_results,
            "assessment_timestamp": datetime.now().isoformat()
        }
        
        logger.info(f"Portfolio risk assessed: {len(loans)} loans, {portfolio_risk:.2f}% risk")
