...
class RecommendationEngine:
    ...
    def __init__(self):
        self.product_catalog = {
            "savings_accounts": [
                {"id": "SA001", "name": "High Yield Savings", "apy": 4.5, "min_balance": 1000, "features": ["high_interest", "online_banking"]},
                {"id": "SA002", "name": "Premium Savings", "apy": 3.8, "min_balance": 5000, "features": ["premium_service", "relationship_banking"]},
                {"id": "SA003", "name": "Basic Savings", "apy": 2.1, "min_balance": 100, "features": ["low_minimum", "basic_banking"]}
            ],
            "checking_accounts": [
                {"id": "CA001", "name": "Premium Checking", "monthly_fee": 0, "min_balance": 2500, "features": ["no_fees", "premium_service", "overdraft_protection"]},
                {"id": "CA002", "name": "Student Checking", "monthly_fee": 0, "min_balance": 0, "features": ["student_benefits", "mobile_banking", "no_fees"]},
                {"id": "CA003", "name": "Basic Checking", "monthly_fee": 10, "min_balance": 500, "features": ["basic_banking", "atm_access"]}
            ],
            "credit_cards": [
                {"id": "CC001", "name": "Cashback Rewards", "apr": 18.9, "annual_fee": 0, "features": ["cashback", "no_annual_fee", "fraud_protection"]},
                {"id": "CC002", "name": "Travel Rewards", "apr": 21.9, "annual_fee": 95, "features": ["travel_rewards", "airport_lounge", "travel_insurance"]},
                {"id": "CC003", "name": "Low Interest", "apr": 12.9, "annual_fee": 0, "features": ["low_apr", "balance_transfer", "no_annual_fee"]}
            ],
            "loans": [
                {"id": "L001", "name": "Personal Loan", "apr": 8.5, "max_amount": 50000, "features": ["fixed_rate", "quick_approval", "no_collateral"]},
                {"id": "L002", "name": "Auto Loan", "apr": 5.2, "max_amount": 100000, "features": ["low_rate", "vehicle_collateral", "flexible_terms"]},
                {"id": "L003", "name": "Home Equity Loan", "apr": 6.8, "max_amount": 500000, "features": ["tax_deductible", "home_collateral", "large_amounts"]}
            ],
            "investments": [
                {"id": "I001", "name": "Conservative Portfolio", "expected_return": 5.5, "risk_level": "low", "features": ["capital_preservation", "steady_income"]},
                {"id": "I002", "name": "Balanced Portfolio", "expected_return": 8.2, "risk_level": "medium", "features": ["growth_income", "diversified"]},
                {"id": "I003", "name": "Growth Portfolio", "expected_return": 11.8, "risk_level": "high", "features": ["capital_growth", "long_term"]}
            ]
        }
    ...
    def recommend_products(self, customer_profile: Dict, limit: int = 5) -> List[Dict]:
        ...
        if emergency_fund_months < 3:
            for product in self.product_catalog['savings_accounts']:
                if credit_score >= 600:  # Basic eligibility
                    score = self._calculate_product_score(product, customer_profile, 'emergency_fund')
        ...
        if savings_rate > 0.15 and emergency_fund_months >= 3:
            for product in self.product_catalog['investments']:
                if (risk_tolerance == 'high' and product['risk_level'] in ['medium', 'high']) or \
                   (risk_tolerance == 'medium' and product['risk_level'] in ['low', 'medium']) or \
                   (risk_tolerance == 'low' and product['risk_level'] == 'low'):
                    score = self._calculate_product_score(product, customer_profile, 'investment')
                    recommendations.append({
                        "product": product,
                        "category": "investment",
                        "score": score,
        ...
        if credit_score >= 650 and 'credit_card' not in current_products:
            for product in self.product_catalog['credit_cards']:
                score = self._calculate_product_score(product, customer_profile, 'credit_card')
                recommendations.append({
                    "product": product,
        ...
        if 'home_purchase' in financial_goals or 'debt_consolidation' in financial_goals:
            for product in self.product_catalog['loans']:
                if credit_score >= 600:  # Basic eligibility
                    score = self._calculate_product_score(product, customer_profile, 'loan')
                    recommendations.append({
                        "product": product,
                        "category": "loan",
                        "score": score,
    ...
    def _calculate_product_score(self, product: Dict, customer_profile: Dict, product_type: str) -> float:
    ...
    def calculate_financial_health_score(self, customer_profile: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "strengths": self._identify_strengths(savings_score, emergency_score, debt_score, credit_score_normalized),
            "areas_for_improvement": self._identify_improvements(savings_score, emergency_score, debt_score, credit_score_normalized)
        }
    
    def _identify_strengths(self, savings_score, emergency_score, debt_score, credit_score) -> List[str]:
    ...
    def _identify_improvements(self, savings_score, emergency_score, debt_score, credit_score) -> List[str]:
...
def recommend_products():
    ...
    try:
        ...
        recommendations = recommendation_engine.recommend_products(customer_profile, limit=5)
        ...
        logger.info(f"Generated {len(recommendations)} product recommendations for customer {data.get('customer_id')}")
...
def get_financial_advice():
    ...
    try:
        ...
        health_score = recommendation_engine.calculate_financial_health_score(customer_profile)
        ...
        logger.info(f"Generated financial advice for customer {data.get('customer_id')}: {health_score['grade']} grade")
...
def get_spending_insights():
    ...
    try:
        ...
        if len(monthly_amounts) >= 2:
            trend = "increasing" if monthly_amounts[-1] > monthly_amounts[-2] else "decreasing"
        else:
            trend = "stable"
        ...
        response = {
            ...
            "top_categories": [{"category": cat, "amount": round(amt, 2), "percentage": round(amt/total_spending*100, 1)} for cat, amt in top_categories],
            "monthly_trend": trend,
        ...
        logger.info(f"Generated spending insights for customer {data.get('customer_id')}")
